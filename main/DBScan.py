...
for channel in range (0,3):
    audio_librosa, sr_librosa = librosa.load(
        'F:/Neural Systems Project/Birdstuff/b8p2male-b10o15female_aligned/2018-08-14/b8p2male-b10o15female_9_SdrChannels.w64',
        sr=None, mono=False)
    print("lobrosa shape: ", audio_librosa.shape)
    print("Now working on channel: ", channel)
    audio_librosa = audio_librosa[channel, int(start_point*sr_librosa):int(end_point*sr_librosa)]
    print(sr_librosa)
    print("lobrosa shape: ", audio_librosa.shape)
    audio_librosa = np.asfortranarray(audio_librosa)
    """
    Sopectrum
    """
    ...
    pre_emphasis = 0.97
    emphasized_audio = np.append(audio_librosa[0], audio_librosa[1:] - pre_emphasis * audio_librosa[:-1])
    print(emphasized_audio.shape)
    # produce frames
    frame_size = 0.025
    frame_stride = 0.01
    frame_length, frame_step = frame_size * sr_librosa, frame_stride * sr_librosa  # Convert from seconds to samples
    ...
    pad_signal = np.append(emphasized_audio,
    ...
    Finding eps
    """
    nbrs = NearestNeighbors(n_neighbors=35, algorithm='ball_tree').fit(total_features)
    ...
    plt.plot(distances)
    plt.show()
    kneedle = KneeLocator(indices[:, 0], distances, curve='convex', direction='increasing')
    print('y-value of knee: ', kneedle.knee_y)
    mean_dist -= (10+(channel*10))
    print(mean_dist)
    """
    DBScan
    """
    #kneedle.knee_y
    model = DBSCAN(eps=mean_dist, min_samples=33)
    ...
    Labeled Audio
    """
    plt.scatter(t,audio[:,channel],c=expanded_db_coeff, linestyle='-', linewidths=0.5)
    plt.show()
    print(dbscan_dict)
    ...
    for i in range(len(dbscan_keys)):
        value = dbscan_keys[i]
        audio_title = 'audio_snippet_' + str(channel) + '_' + str(value) +'.wav'
        print(audio_title)
