...
path_to_data = '/run/media/loaloa/Samsung_T5/b8p2male-b10o15female_aligned/'
output = './..'
...
# make a dataframe that has the recording id as rows, and the filetypes as 
# columns; each entry is the path to the respective file so you can easily get 
# the files you need.
filetypes = ['DAQmx', 'SdrCarrierFreq', 'SdrChannelList', 'SdrChannels', 
             'SdrReceiveFreq', 'SdrSignalStrength','log']
data_files = pd.DataFrame(index=pd.Index(recording_ids, name='rec_id'), 
                          columns=filetypes)
