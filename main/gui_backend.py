...
filt_data_files_path = '/run/media/loaloa/Samsung_T5/filt_birdsong_data'
thr = .05
pad = .5
...
filt_data_files = f'{filt_data_files_path}/filt_data_files_{this_thr}.csv'
if os.path.isfile(filt_data_files):
    filtered_data = pd.read_csv(filt_data_files, index_col='rec_id')
    # merge with standard data
    data_files = pd.concat([data_files, filtered_data], axis=1) 
...
def spectrogram(interactive=True, vmin=-150, vmax=-20, xunit='minutes'):
    ...
    def _spectrogram(vmin, vmax, xunit):
        ...
        fig.subplots_adjust(hspace=0, right=.96, top=.9, left=.1)
...
if __name__ == "__main__":
    pick_rec_no(interactive=False, get_filtered=False, rec_no=43)  # select recording

    pick_plot_limits(interactive=False, start=60, length=20)  # slice the data
    spectrogram(interactive=False,xunit='seconds')  # draw the spectrogram
    ...
    # sf.write('../43_unflit.wav', daq['data_sl'], daq['sample_rate']) # save the sound corresponding to the spectrogram
    plt.show()